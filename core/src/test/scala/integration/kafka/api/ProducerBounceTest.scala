...
class ProducerBounceTest extends KafkaServerTestHarness {
  ...
  private class ProducerScheduler extends ShutdownableThread("daemon-producer", false) {
    ...
    val producers = List(
      TestUtils.createNewProducer(brokerList, bufferSize = producerBufferSize / 4, retries = 10, props = Some(producerConfigWithCompression)),
      TestUtils.createNewProducer(brokerList, bufferSize = producerBufferSize / 2, retries = 10, lingerMs = 5000, props = Some(producerConfigWithCompression)),
