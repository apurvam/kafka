...
class Replica(val brokerId: Int,
   ...
   * to the high watermark if there are no transactional messages in the log.
   */
  def lastStableOffset = log.map(_.firstUnstableOffset.getOrElse(highWatermark.messageOffset))
